package com.desafio.android.core.factory
...
class NetworkResponseCall<T>(
                    ...
                    code == 403 || code == 401 -> {
                        NetworkResponse.Error(
                            exception = Failure.RequestError(
                                msg = "Forbidden: API Key is limited",
                                url = url.toString()
                            )
                        )
                    }

                    else -> responseError(error, code, url)
                }
    ...
    private fun responseError(
        ...
        val errorContent = error.toString()

        return if (errorContent.isNotEmpty()) {
            NetworkResponse.Error(
                exception = Failure.RequestError(
                    httpCode.toString(),
                    errorContent,
